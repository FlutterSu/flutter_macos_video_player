...
+ (nullable FLTAbsolutePositionMessage *)nullableFromList:(NSArray *)list {
  return (list) ? [FLTAbsolutePositionMessage fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    (self.textureId ?: [NSNull null]),
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    case 128: 
      return [FLTAbsolutePositionMessage fromList:[self readValue]];
    case 129: 
...
void FLTVideoPlayerMacOsApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTVideoPlayerMacOsApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.VideoPlayerMacOsApi.absolutePosition"
        binaryMessenger:binaryMessenger
        codec:FLTVideoPlayerMacOsApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(absolutePosition:error:)], @"FLTVideoPlayerMacOsApi api (%@) doesn't respond to @selector(absolutePosition:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTTextureMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
